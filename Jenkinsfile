pipeline {
    ...
    stages {
        stage('Checkout Source') {
            steps {
                git credentialsId: 'githucredentials',
        ...
        stage('Update K8S Manifest and Push to Repo') {
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'githucredentials', passwordVariable: 'GIT_TOKEN', usernameVariable: 'GIT_USERNAME')]) {
                        sh '''
